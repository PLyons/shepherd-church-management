...
import { Member, MemberDocument, COLLECTIONS, QueryOptions } from '../../types/firestore';
import { memberDocumentToMember, memberToMemberDocument } from '../../utils/firestore-converters';
...
export class MembersService extends BaseFirestoreService<MemberDocument, Member> {
   ...
   */
  async createWithAuthUID(authUID: string, memberData: Partial<Member>): Promise<Member> {
   ...
   */
  async getByStatus(status: 'active' | 'inactive' | 'visitor'): Promise<Member[]> {
  ...
  async getMemberDirectory(options?: {
    ...
    const queryOptions: QueryOptions = {
      ...
      orderBy: {
        field: options?.orderBy === 'name' ? 'fullName' : options?.orderBy || 'fullName',
    ...
    // Add filters
    if (options?.status) {
      queryOptions.where!.push({ field: 'memberStatus', operator: '==', value: options.status });
    }
    
    if (options?.role) {
      queryOptions.where!.push({ field: 'role', operator: '==', value: options.role });
    }
    
    if (options?.householdId) {
      queryOptions.where!.push({ field: 'householdId', operator: '==', value: options.householdId });
    }
    ...
    if (options?.search) {
      results = results.filter(member => 
        member.firstName.toLowerCase().includes(options.search!.toLowerCase()) ||
        member.lastName.toLowerCase().includes(options.search!.toLowerCase()) ||
   ...
   */
  async updateWithHouseholdSync(id: string, data: Partial<Member>): Promise<Member> {
    ...
    if (data.householdId && data.householdId !== currentMember.householdId) {
      ...
      if (currentMember.householdId) {
        await this.householdsService.removeMemberFromHousehold(currentMember.householdId, id);
      }
  ...
  async deleteWithHouseholdCleanup(id: string): Promise<void> {
    ...
    if (member.householdId) {
      await this.householdsService.removeMemberFromHousehold(member.householdId, id);
    }
   ...
   */
  async importMembers(membersData: Partial<Member>[]): Promise<{ success: Member[], errors: { data: Partial<Member>, error: string }[] }> {
  ...
  async getStatistics(): Promise<{
    ...
    const [
      ...
      this.count(),
      this.count({ where: [{ field: 'memberStatus', operator: '==', value: 'active' }] }),
      this.count({ where: [{ field: 'memberStatus', operator: '==', value: 'inactive' }] }),
      this.count({ where: [{ field: 'memberStatus', operator: '==', value: 'visitor' }] }),
      this.count({ where: [{ field: 'role', operator: '==', value: 'admin' }] }),
      this.count({ where: [{ field: 'role', operator: '==', value: 'pastor' }] }),
      this.count({ where: [{ field: 'role', operator: '==', value: 'member' }] }),
    ]);
   ...
   */
  subscribeToHouseholdMembers(householdId: string, callback: (members: Member[]) => void): () => void {
