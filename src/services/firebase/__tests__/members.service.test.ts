...
describe('MembersService', () => {
  ...
  describe('error handling', () => {
    it('should handle errors when getting member by email', async () => {
      ...
      vi.spyOn(service, 'getWhere').mockRejectedValue(
  ...
  describe('bulk operations', () => {
    it('should import members successfully', async () => {
      ...
      createSpy
        .mockResolvedValueOnce(
          createMockMember({ id: 'created-1', firstName: 'John' })
        )
        .mockResolvedValueOnce(
          createMockMember({ id: 'created-2', firstName: 'Jane' })
        );
    ...
    it('should handle import validation errors', async () => {
      ...
      vi.spyOn(service, 'getByEmail').mockResolvedValue(
        createMockMember({ email: 'existing@example.com' })
      );
  ...
  describe('household sync operations', () => {
    it('should update member with household sync', async () => {
      ...
      const result = await service.updateWithHouseholdSync(
        'member-1',
        updateData
      );
    ...
    it('should delete member with household cleanup', async () => {
      ...
      expect(
        mockHouseholdsService.removeMemberFromHousehold
      ).toHaveBeenCalledWith('household-1', 'member-1');
    });
  });
  ...
  describe('subscriptions', () => {
    it('should subscribe to member directory', () => {
      ...
      vi.spyOn(service, 'subscribeToCollection').mockReturnValue(
        mockUnsubscribe
      );

      const unsubscribe = service.subscribeToMemberDirectory(
    ...
    it('should subscribe to household members', () => {
      ...
      const unsubscribe = service.subscribeToHouseholdMembers(
        'household-1',
        callback
      );
      ...
      expect(service.subscribeToCollection).toHaveBeenCalledWith(
        expect.objectContaining({
          where: [
            { field: 'householdId', operator: '==', value: 'household-1' },
          ],
          orderBy: { field: 'fullName', direction: 'asc' },
        }),
        callback
