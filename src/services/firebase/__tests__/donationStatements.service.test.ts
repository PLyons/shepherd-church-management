...
describe('DonationStatementsService', () => {
  ...
  describe('Annual Statement Generation', () => {
    it('should generate annual statement for member with donations', async () => {
      ...
      const result = await statementsService.generateAnnualStatement('member-456', 2025);
    ...
    it('should generate statement with multiple categories breakdown', async () => {
      const multipleDonations = [
        { ...mockDonation, categoryName: 'Tithes', amount: 500.00 },
        { ...mockDonation, id: 'donation-456', categoryName: 'Building Fund', amount: 250.00 },
      ];
      ...
      expect(mockCreate).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          categoryBreakdown: expect.arrayContaining([
            { categoryName: 'Tithes', amount: 500.00 },
            { categoryName: 'Building Fund', amount: 250.00 }
          ])
        })
      );
    });
  ...
  describe('Individual Receipt Generation', () => {
    it('should generate receipt for valid donation', async () => {
      ...
      const result = await statementsService.generateDonationReceipt('donation-123');
    ...
    it('should ensure receipt numbering consistency and handle quid pro quo', async () => {
      const quidProQuoDonation = {
        ...mockDonation,
        form990Fields: { isQuidProQuo: true, quidProQuoValue: 25.00 }
      };
  ...
  describe('Template Management', () => {
    it('should get and update templates with validation', async () => {
      const mockTemplate = {
        ...
        bodyContent: { greeting: 'Dear {memberName}', introText: 'Thank you for your support.' }
      };
      
      mockGetWhere.mockResolvedValue([mockTemplate]);
      mockUpdate.mockResolvedValue({ ...mockTemplate, subject: 'Updated Subject' });
      ...
      expect(mockUpdate).toHaveBeenCalledWith('template-id', expect.objectContaining({
        subject: 'Updated Subject',
    ...
    it('should update template with validation', async () => {
      mockUpdate.mockResolvedValue({
        id: 'template-123',
        bodyContent: { greeting: 'Dear {memberName}', introText: 'Updated content for {memberName}' }
      });
    ...
    it('should validate template content requirements', async () => {
      ...
      ).rejects.toThrow('Template must contain required placeholders: {memberName}');
    });
  });
  ...
  describe('Bulk Processing', () => {
    it('should start and track bulk statement jobs', async () => {
      ...
      const result = await statementsService.startBulkStatementJob(2025, 'admin-456');
  ...
  describe('Email Status Tracking', () => {
    it('should mark statements and receipts as emailed', async () => {
      ...
      const result = await statementsService.markStatementEmailSent('statement-123');
    ...
    it('should mark receipts as emailed', async () => {
      ...
      const result = await statementsService.markReceiptEmailSent('receipt-123');
    ...
    it('should validate email delivery status', async () => {
      ...
      ).rejects.toThrow('Email delivery validation failed: statement not marked as sent');
    });
  });
});