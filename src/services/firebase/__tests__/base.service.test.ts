...
class TestFirestoreService extends BaseFirestoreService<TestDocument, TestClient> {
  constructor() {
  ...
  protected clientToDocument(client: Partial<TestClient>): Partial<TestDocument> {
    return {
      name: client.name,
      value: client.value,
    }
...
describe('BaseFirestoreService', () => {
  ...
  describe('create', () => {
    it('should create a new document with auto-generated ID', async () => {
      ...
      const mockDocData = {
        name: 'Test Item',
        value: 42,
        createdAt: { seconds: 1640995200, nanoseconds: 0, toDate: () => new Date('2022-01-01') },
        updatedAt: { seconds: 1640995200, nanoseconds: 0, toDate: () => new Date('2022-01-01') },
      }
    ...
    it('should handle creation errors', async () => {
      vi.mocked(firestore.addDoc).mockRejectedValue(new Error('Creation failed'))

      await expect(service.create({ name: 'Test', value: 1 })).rejects.toThrow('Creation failed')
    })
  })
  ...
  describe('error handling', () => {
    it('should handle Firestore permission errors', async () => {
      const permissionError = new firestore.FirestoreError('permission-denied', 'Permission denied')
      
      vi.mocked(firestore.getDoc).mockRejectedValue(permissionError)

      await expect(service.getById('test-id')).rejects.toThrow('You do not have permission to perform this operation')
    })

    it('should handle Firestore not-found errors', async () => {
      const notFoundError = new firestore.FirestoreError('not-found', 'Document not found')
      
      vi.mocked(firestore.getDoc).mockRejectedValue(notFoundError)

      await expect(service.getById('test-id')).rejects.toThrow('The requested document was not found')
    })
  })
