...
vi.mock('../../utils/firestore-converters', () => ({
  ...
  eventRSVPDocumentToEventRSVP: vi.fn((id: string, doc: any) => ({ id, ...doc })),
  eventRSVPToEventRSVPDocument: vi.fn((rsvp: any) => rsvp),
  eventAttendanceDocumentToEventAttendance: vi.fn((id: string, doc: any) => ({ id, ...doc })),
  eventAttendanceToEventAttendanceDocument: vi.fn((attendance: any) => attendance),
}))
...
describe('EventsService', () => {
  ...
  const createMockAttendance = (overrides: Partial<EventAttendance> = {}): EventAttendance => ({
    id: 'attendance-1',
    memberId: 'member-1',
    memberName: 'John Doe',
    attended: true,
  ...
  describe('public events', () => {
    beforeEach(() => {
      const events = [
        createMockEvent({ id: 'public-1', title: 'Public Event 1', isPublic: true }),
        createMockEvent({ id: 'private-1', title: 'Private Event 1', isPublic: false }),
        createMockEvent({ id: 'public-2', title: 'Public Event 2', isPublic: true }),
      ]
    ...
    it('should get only public events', async () => {
      ...
      expect(result.map(e => e.title).sort()).toEqual(['Public Event 1', 'Public Event 2'])
    })
  })
  ...
  describe('date range queries', () => {
    ...
    it('should get only public events in date range', async () => {
      ...
      const result = await service.getEventsByDateRange(startDate, endDate, true)
  ...
  describe('upcoming and past events', () => {
    ...
    it('should get upcoming events', async () => {
      ...
      expect(result).toHaveLength(2)
      expect(result.map(e => e.id).sort()).toEqual(['upcoming-1', 'upcoming-2'])
    })
  ...
  describe('event search', () => {
    ...
    it('should search events by description', async () => {
      ...
      expect(result).toHaveLength(2)
      expect(result.map(e => e.title).sort()).toEqual(['Sunday Service', 'Youth Group'])
    })
    ...
    it('should search events by location', async () => {
      ...
      expect(result).toHaveLength(2)
      expect(result.map(e => e.title).sort()).toEqual(['Bible Study', 'Youth Group'])
    })
  ...
  describe('events by creator', () => {
    beforeEach(() => {
      ...
      const events = [
        createMockEvent({ id: 'event-1', title: 'Event 1', createdBy: 'user-1' }),
        createMockEvent({ id: 'event-2', title: 'Event 2', createdBy: 'user-2' }),
        createMockEvent({ id: 'event-3', title: 'Event 3', createdBy: 'user-1' }),
      ]
  ...
  describe('RSVP management', () => {
    ...
    it('should get event RSVPs', async () => {
      const mockRSVPs = [
        { id: 'rsvp-1', data: () => ({ memberId: 'member-1', response: 'yes' }) },
        { id: 'rsvp-2', data: () => ({ memberId: 'member-2', response: 'no' }) },
      ]
    ...
    it('should update RSVP', async () => {
      ...
      vi.spyOn(service as any, 'updateEventRSVPStats').mockResolvedValue(undefined)
  ...
  describe('attendance management', () => {
    ...
    it('should get event attendance', async () => {
      const mockAttendance = [
        { id: 'member-1', data: () => ({ memberId: 'member-1', attended: true }) },
        { id: 'member-2', data: () => ({ memberId: 'member-2', attended: false }) },
      ]
    ...
    it('should mark attendance', async () => {
      ...
      const result = await service.markAttendance('event-1', 'member-1', attendanceData)
  ...
  describe('bulk attendance operations', () => {
    it('should bulk mark attendance from RSVPs', async () => {
      const mockRSVPs = [
        createMockRSVP({ memberId: 'member-1', memberName: 'John Doe', response: 'yes' }),
        createMockRSVP({ memberId: 'member-2', memberName: 'Jane Smith', response: 'yes' }),
        createMockRSVP({ memberId: 'member-3', memberName: 'Bob Johnson', response: 'no' }),
      ]
      ...
      // Mock markAttendance
      vi.spyOn(service, 'markAttendance').mockResolvedValue(createMockAttendance())

      const result = await service.bulkMarkAttendanceFromRSVPs('event-1', 'admin-1')
  ...
  describe('calendar operations', () => {
    ...
    it('should check for scheduling conflicts', async () => {
      ...
      vi.spyOn(service, 'getEventsByDateRange').mockResolvedValue(conflictingEvents)
  ...
  describe('statistics', () => {
    ...
    it('should get event statistics', async () => {
      ...
      vi.spyOn(service, 'getUpcomingEvents').mockResolvedValue([createMockEvent()])
      vi.spyOn(service, 'getPastEvents').mockResolvedValue([createMockEvent(), createMockEvent()])
      vi.spyOn(service, 'getEventsByDateRange').mockResolvedValue([createMockEvent()])
  ...
  describe('subscriptions', () => {
    ...
    it('should subscribe to event attendance', () => {
      ...
      const unsubscribe = service.subscribeToEventAttendance('event-1', callback)
  ...
  describe('error handling', () => {
    it('should handle errors when getting RSVPs', async () => {
      ...
      await expect(service.getEventRSVPs('event-1')).rejects.toThrow('Database error')
    })

    it('should handle errors when marking attendance', async () => {
      vi.mocked(setDoc).mockRejectedValue(new Error('Database error'))

      await expect(service.markAttendance('event-1', 'member-1', { attended: true })).rejects.toThrow('Database error')
    })
  })
})