...
export function getStripeConfig(): import('../../types/donations').StripeConfig {
  const config = {
    ...
    minDonationAmount: parseInt(
      import.meta.env.VITE_STRIPE_MIN_DONATION_AMOUNT || '100',
      10
    ),
    maxDonationAmount: parseInt(
      import.meta.env.VITE_STRIPE_MAX_DONATION_AMOUNT || '1000000',
      10
    ),
    allowedCountries: import.meta.env.VITE_STRIPE_ALLOWED_COUNTRIES?.split(
      ','
    ) || ['US'],
  ...
  if (config.minDonationAmount < 50) {
    throw new Error(
      'Minimum donation amount must be at least $0.50 (50 cents)'
    );
  }
