...
export default function PendingRegistrations() {
  ...
  const [stats, setStats] = useState<RegistrationStats>({ total: 0, pending: 0, approved: 0, rejected: 0 });
  const [selectedRegistrations, setSelectedRegistrations] = useState<string[]>([]);
  const [expandedRegistration, setExpandedRegistration] = useState<string | null>(null);
  const [duplicates, setDuplicates] = useState<{ [key: string]: PendingRegistration[] }>({});
    ...
    try {
      ...
      for (const reg of sortedRegs) {
        if (reg.email || reg.phone) {
          const dups = await publicRegistrationService.detectDuplicates(reg.email, reg.phone);
          if (dups.length > 1) {
  ...
  const applyFilters = () => {
    ...
    if (filters.status !== 'all') {
      filtered = filtered.filter(reg => reg.approvalStatus === filters.status);
    }
    ...
    if (filters.memberStatus !== 'all') {
      filtered = filtered.filter(reg => reg.memberStatus === filters.memberStatus);
    }
    ...
    if (filters.dateRange !== 'all') {
      ...
      filtered = filtered.filter(reg => 
        new Date(reg.submittedAt) >= startDate
      );
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <div>
              <select
                ...
                onChange={(e) => setFilters(prev => ({ ...prev, memberStatus: e.target.value as any }))}
                className="w-full py-2 px-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {filteredRegistrations.length === 0 ? (
          ...
          <div className="space-y-4">
            {/* Select All */}
            {filteredRegistrations.some(reg => reg.approvalStatus === 'pending') && (
              <div className="bg-white rounded-lg border p-4">
                <label className="flex items-center">
                  <input
                    type="checkbox"
                    checked={selectedRegistrations.length === filteredRegistrations.filter(reg => reg.approvalStatus === 'pending').length}
                    onChange={handleSelectAll}
                    className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
                  <span className="ml-2 text-sm text-gray-700">
                    Select all pending registrations ({filteredRegistrations.filter(reg => reg.approvalStatus === 'pending').length})
                  </span>
                </label>
              </div>
            )}
            ...
            {filteredRegistrations.map((registration) => (
              <div key={registration.id} className="bg-white rounded-lg border shadow-sm">
                <div className="p-6">
                    ...
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-3">
                          ...
                          {/* Status Badge */}
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(registration.approvalStatus)}`}>
                            {getStatusIcon(registration.approvalStatus)}
                            <span className="ml-1 capitalize">{registration.approvalStatus}</span>
                          </span>

                          {/* Member Status Badge */}
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            registration.memberStatus === 'member' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'
                          }`}>
                        ...
                        <div className="mt-2 text-sm text-gray-600">
                          <span className="font-medium">Event:</span> {tokens[registration.tokenId] || 'Unknown Event'}
                        </div>
                        ...
                        {registration.address?.line1 && (
                          <div className="mt-2 flex items-start text-sm text-gray-600">
                            ...
                            <div>
                              ...
                              <br />
                              {registration.address.city}, {registration.address.state} {registration.address.postalCode}
                            </div>
                          </div>
                        )}
                      </div>
                          ...
                          <dl className="space-y-2 text-sm">
                            {registration.birthdate && (
                              <div>
                                ...
                                <dd className="text-gray-900">{new Date(registration.birthdate).toLocaleDateString()}</dd>
                              </div>
                            )}
                            {registration.gender && (
                              <div>
                                <dt className="text-gray-600">Gender:</dt>
                                <dd className="text-gray-900">{registration.gender}</dd>
                              </div>
                            )}
                          </dl>
                        </div>
                        ...
                        <div>
                          ...
                          <dl className="space-y-2 text-sm">
                            <div>
                              ...
                              <dd className="text-gray-900 break-all">{registration.userAgent}</dd>
                            </div>
                            {registration.ipAddress && (
                              <div>
                                <dt className="text-gray-600">IP Address:</dt>
                                <dd className="text-gray-900">{registration.ipAddress}</dd>
                              </div>
                            )}
                            {registration.approvedBy && (
                              <div>
                                <dt className="text-gray-600">Processed By:</dt>
                                <dd className="text-gray-900">{registration.approvedBy}</dd>
                              </div>
                            )}
                            {registration.approvedAt && (
                              <div>
                                <dt className="text-gray-600">Processed At:</dt>
                                <dd className="text-gray-900">{formatDate(registration.approvedAt)}</dd>
                              </div>
                            )}
                            {registration.rejectionReason && (
                              <div>
                                <dt className="text-gray-600">Rejection Reason:</dt>
                                <dd className="text-red-600">{registration.rejectionReason}</dd>
                              </div>
                            )}
                          </dl>
                        </div>
                      ...
                      {duplicates[registration.id] && (
                        <div className="mt-6 p-4 bg-orange-50 border border-orange-200 rounded-lg">
                          <div className="flex items-start">
                            ...
                            <div className="ml-3">
                              <h4 className="text-sm font-medium text-orange-800">Potential Duplicates Detected</h4>
                              <p className="mt-1 text-sm text-orange-700">
                                Found {duplicates[registration.id].length} registration(s) with matching email or phone number.
                              </p>
                              <div className="mt-2 space-y-1">
                                {duplicates[registration.id].map(dup => (
                                  <div key={dup.id} className="text-sm text-orange-700">
                                    • {dup.firstName} {dup.lastName} - {dup.email || dup.phone} ({dup.approvalStatus})
                                  </div>
                                ))}
                              </div>
                            </div>
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Reject Registration</h3>
            <p className="text-sm text-gray-600 mb-4">
              Please provide a reason for rejecting this registration. This will help with future processing.
            </p>
            <textarea
