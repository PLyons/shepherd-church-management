  ...
  describe('Payment validation functions', () => {
    describe('validatePaymentAmount', () => {
      it('should validate positive amounts with proper decimal precision', () => {
        expect(validatePaymentAmount(25.0)).toBe(null);
        expect(validatePaymentAmount(100.5)).toBe(null);
      ...
      it('should reject zero and negative amounts', () => {
        expect(validatePaymentAmount(0)).toBe(
          'Payment amount must be greater than $0'
        );
        expect(validatePaymentAmount(-5)).toBe(
          'Payment amount must be greater than $0'
        );
        expect(validatePaymentAmount(-100.5)).toBe(
          'Payment amount must be greater than $0'
        );
      });

      it('should enforce minimum payment amount for card transactions', () => {
        expect(validatePaymentAmount(0.3, 'card')).toBe(
          'Minimum card payment amount is $0.50'
        );
        expect(validatePaymentAmount(0.49, 'card')).toBe(
          'Minimum card payment amount is $0.50'
        );
        expect(validatePaymentAmount(0.5, 'card')).toBe(null);
      });

      it('should enforce minimum payment amount for ACH transactions', () => {
        expect(validatePaymentAmount(0.5, 'ach')).toBe(
          'Minimum ACH payment amount is $1.00'
        );
        expect(validatePaymentAmount(0.99, 'ach')).toBe(
          'Minimum ACH payment amount is $1.00'
        );
        expect(validatePaymentAmount(1.0, 'ach')).toBe(null);
      });

      it('should enforce maximum payment limits', () => {
        expect(validatePaymentAmount(10000)).toBe(null);
        expect(validatePaymentAmount(10001)).toBe(
          'Maximum payment amount is $10,000.00'
        );
        expect(validatePaymentAmount(50000)).toBe(
          'Maximum payment amount is $10,000.00'
        );
      });

      it('should validate decimal precision', () => {
        expect(validatePaymentAmount(25.123)).toBe(
          'Amount cannot have more than 2 decimal places'
        );
        expect(validatePaymentAmount(100.999)).toBe(
          'Amount cannot have more than 2 decimal places'
        );
    ...
    describe('validateCreditCardNumber', () => {
      ...
      it('should reject invalid card number formats', () => {
        expect(validateCreditCardNumber('1234567890123456')).toBe(
          'Invalid credit card number'
        );
        expect(validateCreditCardNumber('4242424242424241')).toBe(
          'Invalid credit card number'
        );
        expect(validateCreditCardNumber('123')).toBe(
          'Invalid credit card number'
        );
      });

      it('should reject cards with invalid lengths', () => {
        expect(validateCreditCardNumber('42424242424242')).toBe(
          'Invalid credit card number'
        );
        expect(validateCreditCardNumber('424242424242424212345')).toBe(
          'Invalid credit card number'
        );
      });

      it('should handle empty input', () => {
        expect(validateCreditCardNumber('')).toBe(
    ...
    describe('validateExpiryDate', () => {
      ...
      it('should reject past expiry dates', () => {
        expect(validateExpiryDate(1, currentYear - 1)).toBe(
      ...
      it('should reject past months in current year', () => {
        if (currentMonth > 1) {
          expect(validateExpiryDate(currentMonth - 1, currentYear)).toBe(
            'Credit card has expired'
          );
        }
      });
      ...
      it('should reject invalid month values', () => {
        expect(validateExpiryDate(0, currentYear + 1)).toBe(
          'Invalid expiry month'
        );
        expect(validateExpiryDate(13, currentYear + 1)).toBe(
          'Invalid expiry month'
        );
        expect(validateExpiryDate(-1, currentYear + 1)).toBe(
          'Invalid expiry month'
      ...
      it('should reject invalid year values', () => {
        ...
        expect(validateExpiryDate(12, currentYear + 21)).toBe(
    ...
    describe('validateCVV', () => {
      ...
      it('should reject wrong length CVV for card type', () => {
        expect(validateCVV('12', 'visa')).toBe(
          'CVV must be 3 digits for this card type'
        );
        expect(validateCVV('1234', 'visa')).toBe(
          'CVV must be 3 digits for this card type'
        );
        expect(validateCVV('123', 'amex')).toBe(
          'CVV must be 4 digits for American Express'
        );
      });

      it('should reject non-numeric CVV', () => {
        expect(validateCVV('abc', 'visa')).toBe(
          'CVV must contain only numbers'
        );
        expect(validateCVV('12a', 'visa')).toBe(
          'CVV must contain only numbers'
        );
      });
    ...
    describe('validateRoutingNumber', () => {
      ...
      it('should reject invalid routing number lengths', () => {
        expect(validateRoutingNumber('12345')).toBe(
          'Routing number must be 9 digits'
        );
        expect(validateRoutingNumber('1234567890')).toBe(
          'Routing number must be 9 digits'
        );
      });

      it('should reject non-numeric routing numbers', () => {
        expect(validateRoutingNumber('12345678a')).toBe(
          'Routing number must contain only numbers'
        );
        expect(validateRoutingNumber('abc123456')).toBe(
          'Routing number must contain only numbers'
        );
      });

      it('should reject routing numbers that fail checksum', () => {
        expect(validateRoutingNumber('123456789')).toBe(
          'Invalid routing number'
        );
        expect(validateRoutingNumber('999999999')).toBe(
          'Invalid routing number'
        );
      });
    ...
    describe('validateAccountNumber', () => {
      ...
      it('should reject account numbers that are too short', () => {
        expect(validateAccountNumber('123')).toBe(
      ...
      it('should reject account numbers that are too long', () => {
        expect(validateAccountNumber('123456789012345678')).toBe(
          'Account number must be between 4 and 17 digits'
        );
      });

      it('should reject non-numeric account numbers', () => {
        expect(validateAccountNumber('12345abc')).toBe(
          'Account number must contain only numbers'
        );
        expect(validateAccountNumber('abcd1234')).toBe(
          'Account number must contain only numbers'
        );
      });
    ...
    describe('validateRecurringFrequency', () => {
      ...
      it('should reject invalid frequencies', () => {
        expect(validateRecurringFrequency('daily')).toBe(
          'Invalid recurring frequency'
        );
        expect(validateRecurringFrequency('custom')).toBe(
          'Invalid recurring frequency'
        );
        expect(validateRecurringFrequency('')).toBe(
  ...
  describe('Payment data utilities', () => {
    ...
    describe('formatCurrencyForDisplay', () => {
      it('should format USD amounts correctly', () => {
        ...
        expect(formatCurrencyForDisplay(25.5, 'USD')).toBe('$25.50');
