...
export function InlineEditField<T = string>({
    ...
    if (state.isEditing) {
      ...
      if (currentRef.current) {
        ...
        if (
          ...
          currentRef.current instanceof HTMLTextAreaElement
        ) {
  ...
  const renderInput = () => {
    ...
    switch (fieldType) {
      ...
      case 'date':
        return <input ref={inputRef} {...baseInputProps} type="date" />;

      case 'email':
        return <input ref={inputRef} {...baseInputProps} type="email" />;

      case 'phone':
        return <input ref={inputRef} {...baseInputProps} type="tel" />;

      default:
        return <input ref={inputRef} {...baseInputProps} type="text" />;
    }
  };
  ...
  const EditMode = () => (
    <div
      className={`flex items-center gap-2 p-2 rounded-md border border-blue-200 bg-blue-50 ${className}`}
    >
      {Icon && <Icon className="h-4 w-4 text-gray-500" />}
      <div className="flex-1 min-w-0">
        {label && (
