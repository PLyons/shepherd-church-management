...
export function ActivitySummary({ memberId }: ActivitySummaryProps) {
  ...
  const loadSummary = async () => {
    try {
      setLoading(true);
      const summaryData = await activityService.getActivitySummary(
        memberId,
        timeRange
      );
  ...
  const totalActivities = Object.values(summary).reduce(
    (sum, count) => sum + count,
    0
  );
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            value={timeRange}
            onChange={(e) =>
              setTimeRange(Number(e.target.value) as 30 | 90 | 365)
            }
                ...
                {topActivityTypes.map(([type, count]) => {
                  ...
                  return (
                    <div key={type} className="text-center">
                      <div
                        className={`inline-flex items-center justify-center h-8 w-8 rounded-full ${config.bgColor} mb-2`}
                      >
