...
import { Donation, DonationReportFilters, FinancialSummary } from '../../../types/donations';
...
export interface ExportOptions {
  type: 'pdf' | 'csv';
  reportType: 'executive' | 'detailed' | 'form990' | 'category' | 'donor' | 'multi_year';
...
export const ExportControls: React.FC<ExportControlsProps> = ({
  ...
  const [selectedExportType, setSelectedExportType] = useState<'pdf' | 'csv'>('pdf');
  const [selectedReportType, setSelectedReportType] = useState<ExportOptions['reportType']>('executive');
  ...
  const generateExecutiveSummaryPDF = useCallback(async (options: ExportOptions): Promise<Blob> => {
    setExportProgress({ stage: 'Generating executive summary...', progress: 20 });
  ...
  const generateForm990PDF = useCallback(async (options: ExportOptions): Promise<Blob> => {
    setExportProgress({ stage: 'Generating Form 990 report...', progress: 30 });
  ...
  const generateDetailedReportPDF = useCallback(async (options: ExportOptions): Promise<Blob> => {
    setExportProgress({ stage: 'Generating detailed report...', progress: 25 });
  ...
  const generateMultiYearPDF = useCallback(async (options: ExportOptions): Promise<Blob> => {
    setExportProgress({ stage: 'Generating multi-year report...', progress: 35 });
    ...
    setExportProgress({ stage: 'Finalizing multi-year PDF...', progress: 90 });
    return new Blob([mockPDFData], { type: 'application/pdf' });
  }, []);
  ...
  const generateDetailedTransactionsCSV = useCallback(async (options: ExportOptions): Promise<Blob> => {
    setExportProgress({ stage: 'Generating transaction CSV...', progress: 40 });
    ...
    // Filter fields based on user role
    const filteredFields = member?.role === 'admin' 
      ? fields 
      : fields.filter(field => !['memberName', 'memberEmail', 'memberPhone'].includes(field));
    ...
    const csvData = donations.map(donation => ({
      ...
      memberName: member?.role === 'admin' ? donation.memberName || 'Anonymous' : 'REDACTED',
      ...
      restrictionType: donation.form990Fields?.restrictionType || 'unrestricted'
    }));
  ...
  const generateCategorySummaryCSV = useCallback(async (options: ExportOptions): Promise<Blob> => {
    setExportProgress({ stage: 'Generating category summary...', progress: 45 });
    ...
    const mockServiceCall = {
      ...
      excludeFields: member?.role === 'pastor' ? ['memberName', 'memberEmail', 'memberPhone'] : [],
    ...
    donations.forEach(donation => {
      const existing = categoryMap.get(donation.categoryName) || { amount: 0, count: 0 };
    ...
    const csvData = Array.from(categoryMap.entries()).map(([categoryName, data]) => ({
      categoryName,
    ...
    const csv = Papa.unparse(csvData, { header: true, quotes: true, delimiter: ',' });

    setExportProgress({ stage: 'Finalizing category CSV...', progress: 90 });
    return new Blob([csv], { type: 'text/csv' });
  }, [donations, member]);

  const generateDonorReportCSV = useCallback(async (options: ExportOptions): Promise<Blob> => {
    if (member?.role !== 'admin') {
    ...
    const donorMap = new Map<string, { memberName: string, totalAmount: number, donationCount: number }>();
    donations.forEach(donation => {
    ...
    const csvData = Array.from(donorMap.values()).map(donor => ({
      ...
      averageDonation: formatCurrency(donor.totalAmount / donor.donationCount)
    }));
  ...
  const generateForm990CSV = useCallback(async (options: ExportOptions): Promise<Blob> => {
    ...
    const csvData = donations.map(donation => ({
      form990LineItem: donation.form990Fields?.lineItem || '1a_cash_contributions',
      amount: donation.amount.toString(),
      categoryName: donation.categoryName,
      restrictionType: donation.form990Fields?.restrictionType || 'unrestricted',
      quidProQuoValue: donation.form990Fields?.quidProQuoValue?.toString() || '0',
      fairMarketValue: donation.form990Fields?.fairMarketValue?.toString() || donation.amount.toString(),
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      if (selectedExportType === 'pdf') {
        switch (selectedReportType) {
          ...
          case 'multi_year':
            ...
            break;
          default:
            throw new Error(`Unsupported PDF report type: ${selectedReportType}`);
        }
      } else {
        // CSV exports
        switch (selectedReportType) {
          ...
          case 'form990':
            ...
            break;
          default:
            throw new Error(`Unsupported CSV report type: ${selectedReportType}`);
        }
      }
      ...
      showToast(`${selectedReportType} ${selectedExportType.toUpperCase()} exported successfully`, 'success');
      ...
      showToast(`Failed to generate ${selectedExportType.toUpperCase()} export: ${errorMessage}`, 'error');
    } finally {
      setIsExporting(false);
      setExportProgress(null);
    }
  }, [
    member, selectedExportType, selectedReportType, customFields, includeCharts, filters,
    showToast, onExportComplete, generateExecutiveSummaryPDF, generateForm990PDF,
    generateDetailedReportPDF, generateMultiYearPDF, generateDetailedTransactionsCSV,
    generateCategorySummaryCSV, generateDonorReportCSV, generateForm990CSV
  ]);
    ...
    <div className={`bg-white rounded-lg shadow-sm border border-gray-200 p-6 ${className}`}>
      <div className="flex items-center justify-between mb-6">
        ...
        {isExporting && exportProgress && (
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-600">{exportProgress.stage}</span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => setSelectedExportType(e.target.value as 'pdf' | 'csv')}
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => setSelectedReportType(e.target.value as ExportOptions['reportType'])}
            disabled={isExporting}
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          >
      ...
      {selectedExportType === 'csv' && (
        <div className="mt-6">
          <div className="flex items-center justify-between mb-3">
            ...
            >
              {showCustomFields ? 'Use Default Fields' : 'Custom Field Selection'}
            </button>
          </div>

          {showCustomFields && (
            <div data-testid="csv-field-selector" className="grid grid-cols-2 md:grid-cols-3 gap-2 p-4 bg-gray-50 rounded-md">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setCustomFields(customFields.filter(f => f !== field.id));
                      }
                    }}
                    disabled={isExporting}
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
      ...
      <div className="mt-6 p-3 bg-gray-50 rounded-md">
        <p className="text-xs text-gray-600">
          {member?.role === 'admin' && (
            <>
              <strong>Administrator Access:</strong> Full export capabilities including individual donor information and sensitive financial data.
            </>
          )}
          {member?.role === 'pastor' && (
            <>
              <strong>Pastor Access:</strong> Aggregate reports available. Individual donor details excluded for privacy protection.
            </>
          )}
