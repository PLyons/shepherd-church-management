...
export function TrendChart({
  ...
  const processMonthlyData = (
    ...
    return months.map((month) => {
      ...
      const previousMonthDonations = donations.filter((donation) => {
        const donationDate = parseISO(donation.donationDate);
        const previousYearMonth = new Date(
        ...
        return (
          donationDate >= previousYearMonth &&
          donationDate <= previousYearMonthEnd
        );
      });

      const currentTotal = currentMonthDonations.reduce(
        (sum, d) => sum + d.amount,
        0
      );
      const previousTotal = previousMonthDonations.reduce(
        (sum, d) => sum + d.amount,
        0
      );
  ...
  const processQuarterlyData = (
    ...
    return quarters.map((quarter, index) => {
      ...
      const previousQuarterDonations = donations.filter((donation) => {
        const donationDate = parseISO(donation.donationDate);
        return (
          donationDate.getFullYear() === currentYear - 1 &&
          months.includes(donationDate.getMonth())
        );
      });

      const currentTotal = currentQuarterDonations.reduce(
        (sum, d) => sum + d.amount,
        0
      );
      const previousTotal = previousQuarterDonations.reduce(
        (sum, d) => sum + d.amount,
        0
      );
  ...
  const processYearlyData = (
    ...
    return years.map((year) => {
      const yearDonations = donations.filter(
        (d) => parseISO(d.donationDate).getFullYear() === year
      );

      const previousYearDonations = donations.filter(
        (d) => parseISO(d.donationDate).getFullYear() === year - 1
      );

      const total = yearDonations.reduce((sum, d) => sum + d.amount, 0);
      const previousTotal = previousYearDonations.reduce(
        (sum, d) => sum + d.amount,
        0
      );
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex rounded-md shadow-sm">
            {(['monthly', 'quarterly', 'yearly'] as TimeFrame[]).map(
              (frame) => (
                <button
                  key={frame}
                  type="button"
      ...
      <div
        style={{ height, minHeight: 300, width: '100%', minWidth: 400 }}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
          <div className="text-center">
            <p className="font-medium text-gray-900">
              {formatCurrency(
                trendData.reduce((sum, d) => sum + d.currentYear, 0)
              )}
            </p>
          ...
          {showComparison && (
            <div className="text-center">
              <p className="font-medium text-gray-900">
                {formatCurrency(
                  trendData.reduce((sum, d) => sum + d.previousYear, 0)
                )}
              </p>
