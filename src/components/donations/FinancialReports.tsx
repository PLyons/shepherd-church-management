...
export const FinancialReports: React.FC<FinancialReportsProps> = ({ className = '' }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'form990' | 'exports'>('overview');
  ...
  // Role-based access
  const hasAdminAccess = member?.role === 'admin' || (user as any)?.role === 'admin';
  ...
  const loadCategories = useCallback(async () => {
    try {
      ...
      if (donationCategoriesService && typeof donationCategoriesService.getAll === 'function') {
  ...
  const handleExportCategorySummary = async () => {
    try {
      ...
      await donationsService.exportDonationsCSV({
        ...
        excludeFields: member?.role === 'pastor' ? ['memberName', 'memberEmail', 'memberPhone'] : undefined,
  ...
  const categoryChartData = safeFinancialSummary.categoryBreakdown.map((cat, index) => ({
    ...
    return (
      <div className={`bg-white rounded-lg shadow-sm border p-6 ${className}`}>
        <div className="text-center py-8">
          ...
          <p className="text-gray-600">Admin or Pastor access required to view financial reports.</p>
        </div>
      </div>
    );
  }
  ...
  if (isLoading) {
    return (
      <div className={`bg-white rounded-lg shadow-sm border p-6 ${className}`}>
        <span data-testid="loading-spinner" className="sr-only">Loading</span>
  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm border ${className}`} role="main" aria-label="Financial Reports Dashboard">
      {/* Header with Tabs */}
      <div className="px-6 py-4 border-b border-gray-200">
        <h1 className="text-2xl font-bold text-gray-900 mb-4">Financial Reports Dashboard</h1>
      ...
      <div className="p-6">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-8" data-testid="financial-overview">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="bg-blue-50 rounded-lg p-4" data-testid="total-donations-card">
                <h3 className="text-lg font-semibold text-blue-900">Total Donations</h3>
                <p className="text-2xl font-bold text-blue-600">{formatCurrency(safeFinancialSummary.totalDonations)}</p>
                <p className="text-sm text-blue-700">{safeFinancialSummary.donationCount} donations</p>
              </div>
              <div className="bg-green-50 rounded-lg p-4" data-testid="tax-deductible-card">
                <h3 className="text-lg font-semibold text-green-900">Tax Deductible</h3>
                <p className="text-2xl font-bold text-green-600">{formatCurrency(safeFinancialSummary.totalTaxDeductible)}</p>
              </div>
              <div className="bg-purple-50 rounded-lg p-4" data-testid="average-donation-card">
                <h3 className="text-lg font-semibold text-purple-900">Average Donation</h3>
                <p className="text-2xl font-bold text-purple-600">{formatCurrency(safeFinancialSummary.averageDonation)}</p>
              </div>
              <div className="bg-orange-50 rounded-lg p-4" data-testid="categories-card">
                <h3 className="text-lg font-semibold text-orange-900">Categories</h3>
                <p className="text-2xl font-bold text-orange-600">{safeFinancialSummary.categoryBreakdown.length}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div>
                ...
                <div className="h-64" data-testid="pie-chart" style={{ minHeight: 256, minWidth: 400, width: '100%' }}>
              ...
              <div>
                ...
                <div className="h-64" data-testid="line-chart" style={{ minHeight: 256, minWidth: 400, width: '100%' }}>
                  {monthlyTrendData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%" minWidth={400} minHeight={256}>
                      <LineChart data={monthlyTrendData} data-testid="monthly-trends-chart">
            ...
            {/* Custom Export */}
            <div className="mt-6">
              <h4 className="text-md font-semibold text-gray-900 mb-3">Custom Export</h4>
              <div className="bg-gray-50 rounded-lg p-4">
                <p className="text-gray-600 mb-3">Select specific fields for custom export</p>
                <div data-testid="csv-field-selector" className="space-y-2 mb-4">
        ...
        {activeTab === 'exports' && (
          <div className="space-y-8">
            {/* PDF Exports */}
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">PDF Reports</h3>
            ...
            {/* CSV Exports */}
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">CSV & Data Exports</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <button
                  onClick={handleExportTransactionsCSV}
                  disabled={isExporting}
            ...
            <div>
              ...
              <div className="bg-gray-50 rounded-lg p-6">
                <p className="text-gray-600 mb-4">Select specific fields for custom export</p>
