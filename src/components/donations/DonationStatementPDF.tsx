...
export class DonationStatementPDF {
   ...
   */
  private addHeader(
    churchInfo: DonationStatementData['churchInfo'],
    statementNumber: string,
    generatedDate: string
  ): void {
    ...
    this.pdf.text(churchInfo.name, this.pageWidth / 2, this.currentY, {
      align: 'center',
    });
    this.currentY += 25;
    ...
    this.pdf.text(churchInfo.address, this.pageWidth / 2, this.currentY, {
      align: 'center',
    });
    this.currentY += 12;
  ...
  private addStatementPeriod(taxYear: number): void {
    ...
    this.pdf.text(
      `Statement Period: January 1, ${taxYear} - December 31, ${taxYear}`,
      this.margins.left,
      this.currentY + 12
    );
    this.currentY += 30;
   ...
   */
  private addDonationSummary(
    donations: Donation[],
    includeNonDeductible: boolean = false
  ): void {
    ...
    const totalDeductible = deductibleDonations.reduce(
      (sum, d) => sum + d.amount,
      0
    );
    const totalNonDeductible = nonDeductibleDonations.reduce(
      (sum, d) => sum + d.amount,
      0
    );
    ...
    // Summary box
    const summaryHeight =
      includeNonDeductible && totalNonDeductible > 0 ? 80 : 60;
    this.pdf.setFontSize(12);
    this.pdf.setFont('helvetica', 'bold');
  ...
  private addDonationDetails(donations: Donation[]): void {
    ...
    donations.forEach((donation) => {
      ...
      rowData.forEach((data, index) => {
        this.pdf.text(data, currentX, this.currentY, {
  ...
  private addFooter(
    ...
    verificationText.forEach((line, index) => {
      this.pdf.text(line, this.pageWidth / 2, this.currentY + index * 10, {
        align: 'center',
      });
    });
