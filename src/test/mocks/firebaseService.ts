...
import { mockTimestamp, createMockDocSnapshot, createMockQuerySnapshot } from './firebase'
...
export class MockBaseFirestoreService<TDocument, TClient> {
  ...
  subscribeToCollection(options?: any, callback?: (data: TClient[]) => void): () => void {
    // Immediately call with current data
...
export const createMockFirebaseService = <TDocument, TClient>(
  ...
  const service = new MockBaseFirestoreService<TDocument, TClient>(collectionName)
