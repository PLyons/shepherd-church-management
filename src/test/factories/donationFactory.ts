...
export const createMockDonation = (overrides: Partial<MockDonation> = {}): MockDonation => {
  return {
    id: faker.string.uuid(),
    memberId: faker.string.uuid(),
    amount: faker.number.float({ min: 5, max: 1000, multipleOf: 0.01 }),
    date: faker.date.past(),
    method: faker.helpers.arrayElement(['cash', 'check', 'credit_card', 'bank_transfer']),
...
export const createMockDonationsForMember = (memberId: string, count: number = 5): MockDonation[] => {
  return Array.from({ length: count }, () => 
    createMockDonation({ memberId })
  )
}

export const createMockYearlyDonations = (year: number, memberCount: number = 10) => {
