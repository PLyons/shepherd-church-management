...
async function setupAdmin() {
  ...
  try {
    ...
    const setupMode = await question('Are you (1) creating a new admin or (2) promoting an existing user? [1/2]: ');
    ...
    if (setupMode === '2') {
      ...
      const password = await question('Admin password (min 8 chars, uppercase, lowercase, number, special char): ');
